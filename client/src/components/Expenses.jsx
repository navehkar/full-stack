...
export const Expenses = () => {
  ...
  const maxAmount = expenses.length ?Math.max(...expenses.map(expense => expense.amount), 0):0;
  console.log(maxAmount);
  const filteredExpenses = expenses.filter((expense) =>{
  const matchSearch =  expense.title.toLowerCase().includes(inputSearch.toLowerCase())
  if(selectedFilter && selectedFilter.type === 'amount') {return (
    matchSearch&& expense.amount >= selectedFilter.min && expense.amount <= selectedFilter.max
  )}
  return matchSearch;
});
  const resetFields = () => {
